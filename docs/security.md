...
| **BACnet scraper** | **Data model or CSV** | The scraper polls only the points it is configured with: **by default, points in the data model** that have `bacnet_device_id` and `object_identifier` (e.g. added via `POST /bacnet/discovery-to-rdf` with `import_into_data_model: true`, or CRUD). If none exist, it can fall back to a **curated CSV**. Throttling depends on **how many points** are defined (in the DB or in the CSV). Best practice: scrape only the points needed for FDD and HVAC health. See [BACnet overview](bacnet/overview#discovery-and-getting-points-into-the-data-model). |
...
So outbound load on the OT network is predictable and tunable. **Define only the points you need** (in the data model via discovery-to-rdf with import_into_data_model or CRUD, or in a curated CSV), then adjust intervals via `OFDD_*` environment variables. See [Configuration](configuration) and [BACnet overview](bacnet/overview#discovery-and-getting-points-into-the-data-model).
