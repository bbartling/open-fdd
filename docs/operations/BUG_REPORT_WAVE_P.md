...
| **wave-s5-dm-remainder** | **PARTIAL (W4)** · EQ-VOCAB + ECM-ADAPT + Pages vocab CLOSED on tip **4.4.7**; Soft-OPEN residual **DM-09 PERF** (measure-first oxigraph bound) + **DM-10** edge Haystack `projectionVersion` (EQ projection shipped; no product Rust claim) |
...
| **wu-dm-07-10** | Soft-OPEN | **PARTIAL (W4 tip)** | DM-07/08 CLOSED (V5). **CLOSED on W4 tip:** EQ-VOCAB · ECM-ADAPT · Pages vocab (`ofdd_engineering_quantities_v1`, `adapt_model_to_calculator`, pytest). **Soft-OPEN residual:** DM-09 PERF (no oxigraph bench/bound without measured baseline) · DM-10 edge Haystack `projectionVersion` / no-false-marker product path (docs honesty only; do not claim from EQ alone) |
