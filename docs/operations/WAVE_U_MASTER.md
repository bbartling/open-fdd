...
| `wave-s5-dm-remainder` | Soft-OPEN / PARTIAL | DM-07/08 + EQ-VOCAB + ECM-ADAPT + Pages (W4 tip 4.4.7); residual DM-09 PERF + edge DM-10 projection |
...
## After security spine (product Soft-OPEN — next tips)

Do **not** greenwash CLOSED until each tip has GHCR smoke / FQ evidence:

1. `wave-s5-dm-remainder` → VERSION tip → smoke  
2. `wave-s3-pypi-mv-oracle` → smoke  
3. `wave-s4-sql-twins-fq` → GHCR → fieldbus → **MEGA FQ** `OPENFDD_SECURITY_EXECUTE=1`  
4. Closeout: BUG_REPORT OPS PINNED + Soft-OPEN honesty  

Child detail: `wave_s3_*` / `wave_s4_*` / `wave_s5_*` plans (not masters).
