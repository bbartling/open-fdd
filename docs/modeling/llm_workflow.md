...
> **Automated path available:** External OpenAI-compatible agents (for example Open Claw) can automate the same flow by calling `GET /data-model/export`, fetching platform documentation context from `GET /model-context/docs`, and then calling `PUT /data-model/import` with validated import JSON. For HTTP auth, split workspaces, and **SSH-based operator** steps (remote VOLTTRON edge → VOLTTRON Central), see **[Open Claw integration](../openclaw_integration)** — especially **section 1f** there, which points at official VOLTTRON docs. The manual copy-paste workflow below always works too.
...
- **Same names as the UI presets** are maintained in this monorepo under [`afdd_stack/frontend/src/data/brick-1.4-query-class-allowlist.ts`](https://github.com/bbartling/open-fdd/blob/main/afdd_stack/frontend/src/data/brick-1.4-query-class-allowlist.ts) (regression test: [`data-model-testing-queries.brick.test.ts`](https://github.com/bbartling/open-fdd/blob/main/afdd_stack/frontend/src/data/data-model-testing-queries.brick.test.ts)).
