...
- **[AI-assisted data modeling](ai_assisted_tagging)** — Export → LLM or human tagging → import (Brick types, rule_input, polling, equipment feeds). Optional **in-house agent** (Data Model Setup → OpenAI API Assist): uses the same API with retry and **prompt chaining** on validation failure; prompt from `config/canonical_llm_prompt.txt`. See [Technical reference](../appendix/technical_reference) for the full prompt and schema.
