...
- **[External representations (Brick v1.4)](external_representations)** — `ref:hasExternalReference` mappings from points to external systems (metadata / interoperability).
...
- **[SPARQL cookbook](sparql_cookbook)** — Run SPARQL via POST /data-model/sparql: config, data model, optional BACnet-shaped triples, FDD rule mapping, time-series references. Copy-paste queries for validation and UIs.
