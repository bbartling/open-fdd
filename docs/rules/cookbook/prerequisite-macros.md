...
The React SPA-tested catalog wraps many rules with **operational gates** (fan proven-on, occupancy, override suppression, pressure-on proxies). Gates live alongside rule computes in the vibe19 `operational_gate` module — SQL recipes should replicate the same predicates before latching `fault_raw`.
