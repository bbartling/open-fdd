...
4. **Browser / React (optional)** — if you open the UI from your PC against a dev server on the edge box, the frontend needs the same key at **build** time as **`VITE_OFDD_API_KEY`** (see [Security — Frontend API key](security#frontend-api-key-bearer)) and often **`VITE_API_TARGET`** pointing at the API. That is separate from the API Bearer header OpenClaw uses for REST.
