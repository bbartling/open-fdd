...
**This page is intentionally retired.** Open-FDD **does not** host BACnet discovery or scrapers in the default architecture. **Per-building VOLTTRON** owns BACnet (and Modbus) on the **ZMQ** message bus; telemetry reaches Open-FDD through **SQL** and **`external_id`** mapping.
