...
**Verify BACnet scraping:** `curl -s http://localhost:8000/timeseries/latest` (recent `ts` = data flowing). Scraper logs: `docker logs openfdd_bacnet_scraper --tail 30`. Full steps: [Verification — BACnet scraper](verification#bacnet-scraper).
