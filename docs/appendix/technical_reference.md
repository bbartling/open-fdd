...
| `OFDD_API_KEY` | Generated by bootstrap | Optional. When set, all requests (except `/`, `/health`, `/docs`, `/redoc`, `/openapi.json`, `/app`) require `Authorization: Bearer <OFDD_API_KEY>`. Bootstrap generates a key and writes it to `stack/.env` unless you pass `--no-auth`. Use the same value in HA integration and addon `api_key`. |
