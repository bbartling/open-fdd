...
4. **Scraper runs automatically** — The bacnet-scraper container runs `python -m open_fdd.platform.drivers.run_bacnet_scrape`; it loads BACnet addresses from the knowledge graph (`points`) and polls only those. Configuration is via the frontend and data model only.
