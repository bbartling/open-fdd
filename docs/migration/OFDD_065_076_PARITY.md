...
| OFDD-073 | Multi-site React SPA + Hive purge | A | **VERIFIED (bench) / PENDING Liberty soak** | Session sites + Delete site UI verified earlier. Tip also purges `.cache/rule_results/building={id}/` on `DELETE /api/datasets` (OFDD-073 residual from 2026-07-29 soak). Liberty re-soak still required to prove B50 gone / B100 intact. |
