...
| CAP-RULES | Rule catalog, tuning, run-all/selected | rule_card, central_client, streamlit_app | /api/fdd/rules*, /api/fdd/run | RulesPage | fdd_rules + routes | sql_rules/ | EXACT+NUMERIC | fddApi + RulesPage vitest | react_ui | IN_PROGRESS | React run UI (M4-04); deep tuning → M5-A; Streamlit still default |
