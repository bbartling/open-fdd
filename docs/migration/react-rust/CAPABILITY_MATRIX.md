...
| CAP-FINDINGS | Engineering findings + dispositions | eng_findings | /api/jobs/*/findings|dispositions, /api/reports/engineering-findings | FindingsPage | jobs.rs | NONE | EXACT | findingsApi + FindingsPage vitest | react_ui | IN_PROGRESS | Job findings + disposition save (M5-D); FDD results on Rules; Streamlit still default |
| CAP-REPORTS | Report/artifact generation + downloads | reports, report_downloads, docx_report, tuning_report | /api/reports* | ReportsPage (artifacts) | reports routes | NONE | ARTIFACT | reportsApi vitest | react_ui | IN_PROGRESS | List/draft/eng-findings (M5-E); PDF/DOCX may stay ORACLE |
| CAP-WATTLAB | WattLab dump + job-native handoff | wattlab_dump, ui_wattlab_job, ui_wattlab_studio | /api/jobs/*/wattlab/handoffs | WattLabPage | jobs wattlab | NONE | ARTIFACT | WattLabPage vitest | react_ui | IN_PROGRESS | Handoff POST (M5-E); Streamlit still default |
