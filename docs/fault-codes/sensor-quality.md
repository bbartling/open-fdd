...
Sensor faults use cookbook patterns **`flatline_1h`**, **`oob_rolling`**, **`rate_of_change`**, and **`mixing_envelope`**. Full thresholds: [Expression cookbook — sensor validation](../rule-cookbook/expression-cookbook#sensor-validation-bounds-flatline-rate-of-change).
