...
Open-FDD does **not** ship an embedded chatbot. Any AI agent that can read markdown skills and run the PyPI CLI can use this package (Cursor, Claude, Codex, OpenClaw, Hermes, Grok Bot, or another MCP host). `openfdd_agent_spec/skills/` is the only authoring tree ([index](https://github.com/bbartling/open-fdd/tree/master/openfdd_agent_spec/skills)). Do not invent a parallel skill under `.cursor/skills/` or another vendor directory. `./scripts/openfdd_install_agent_skills.sh --sync` (add `--user` for home directories) is the sync into Cursor, Claude, Codex, OpenClaw, Hermes, and Grok. Live stacks still use **JWT REST** and optional **`openfdd-mcp` stdio**. The PDF command is `open-fdd-anomaly report ... --compile`. It is not Railway-only: mapped roles from a local CSV plus column map, a Railway or self-hosted Open-FDD central, or a future vendor API.
