...
for brick_term in brick_mappings.values():
    ...
    if column_name not in df.columns:
        print(
            f"ISSUE! - Column {column_name} is NOT successfully mapped to {brick_term}"
        )
    else:
...
class Query:
    ...
    def average_by_month(self, sensorName: str) -> List[MonthlyAverage]:
        ...
        if column and column in df.columns:
            ...
            return [
                MonthlyAverage(month=row["Date"], average=row[column])
    ...
    def averageByWeek(self, sensorName: str) -> List[WeeklyAverage]:
        ...
        if column and column in df.columns:
            ...
            weekly_data = (
                df.set_index("Date").resample("W")[column].mean().reset_index()
            )
            weekly_data["Date"] = weekly_data["Date"].dt.strftime("%Y-%m-%d")
            return [
                WeeklyAverage(week=row["Date"], average=row[column])
    ...
    def averageByDay(self, sensorName: str) -> List[DailyAverage]:
        ...
        if column and column in df.columns:
            ...
            return [
                DailyAverage(day=row["Date"], average=row[column])
...
query_air_flow_monthly = (
    '{ averageByMonth(sensorName: "Supply_Air_Flow_Sensor") { month average } }'
)
query_outside_air_temp_monthly = (
    '{ averageByMonth(sensorName: "Outside_Air_Temperature_Sensor") { month average } }'
)
...
query_air_flow_weekly = (
    '{ averageByWeek(sensorName: "Supply_Air_Flow_Sensor") { week average } }'
)
...
# Weekly Example usage
query_air_flow_daily = (
    '{ averageByDay(sensorName: "Supply_Air_Flow_Sensor") { day average } }'
)
query_outside_air_temp_daily = (
    '{ averageByDay(sensorName: "Outside_Air_Temperature_Sensor") { day average } }'
)
