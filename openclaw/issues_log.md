...
- **Frontend PlotsPage review:** **Superseded (2026-04).** The note above was wrong for current `PlotsPage.tsx`: the page again selects a BACnet device, passes **`equipment_ids`** into fault-timeseries for overlays, and the fault picker lists state rows tied by **equipment** or by **`bacnet_device_id`** (GitHub #117). Definition names appear in the picker; URL params **`device`** and **`fault`** work with existing **`site`**. Active Faults includes a **Trends** deep link to Plots when BACnet device id is set.
