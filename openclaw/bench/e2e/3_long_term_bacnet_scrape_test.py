...
SITE_HINT_ENV = os.environ.get("OFDD_BACNET_SITE_ID", "").strip()
PRIMARY_SITE_ID: str | None = None
PRIMARY_SITE_NAME: str | None = None
SITE_QUERY_VALUE: str | None = SITE_HINT_ENV or None
_FAULT_SCHEDULE_MOD = None
EXPECTED_FAULT_IDS = {"flatline_flag", "bad_sensor_flag"}
...
def get_first_site_id() -> str:
    """Return cached primary site id when available, else fall back to the first site from GET /sites."""
    if PRIMARY_SITE_ID:
...
def _resolve_primary_site(site_hint: str | None) -> tuple[str, str]:
    """Return (site_id, site_name) matching hint (id/name substring) or the first site."""
    code, body = _request("GET", "/sites")
    if code != 200 or not isinstance(body, list) or not body:
        raise RuntimeError("GET /sites did not return any sites; create a site first.")
    if site_hint:
        ...
        for entry in body:
            ...
            sdesc = str(entry.get("description", "")).strip()
            if target in {sid.lower(), sname.lower(), sdesc.lower()}:
                return sid, sname or sid
    first = body[0]
    sid = str(first.get("id"))
    if not sid:
        raise RuntimeError("First site entry missing id; cannot continue.")
    return sid, (first.get("name") or sid)


def _get_fault_schedule_module():
    """Lazy-load fake_bacnet_devices/fault_schedule.py so both devices and tests share the same schedule."""
    global _FAULT_SCHEDULE_MOD
    if _FAULT_SCHEDULE_MOD is not None:
        return _FAULT_SCHEDULE_MOD
    script_dir = Path(__file__).resolve().parent
    fake_dev_dir = script_dir.parent / "fake_bacnet_devices"
    if fake_dev_dir.is_dir():
        path_str = str(fake_dev_dir)
        if path_str not in sys.path:
            sys.path.insert(0, path_str)
    try:
        import fault_schedule  # type: ignore
    except ImportError:
        return None
    _FAULT_SCHEDULE_MOD = fault_schedule
    return _FAULT_SCHEDULE_MOD


def _fault_id_for_mode(mode: str | None) -> str | None:
    if mode == "flatline":
        return "flatline_flag"
    if mode == "bounds":
        return "bad_sensor_flag"
    return None


def get_download_faults(site_identifier: str | None, start_d: date, end_d: date) -> tuple[list[dict], str]:
...
def get_download_csv_distinct_timestamps(site_identifier: str | None, start_d: date, end_d: date) -> tuple[int, str]:
...
def _check_expected_faults_for_phase(phase_end_utc: datetime, site_identifier: str | None) -> tuple[bool, str]:
    """
    ...
    Returns (ok, message).
    """
    sched_mod = _get_fault_schedule_module()
    if sched_mod is None:
        return False, "fault_schedule module not available (fake_bacnet_devices missing?)"
    ...
    faults, err = get_download_faults(site_identifier, start_d, end_d)
    if err:
        return False, f"Faults fetch failed: {err}"

    windows = sched_mod.expected_fault_windows_utc(start_utc, phase_end_utc)
...
def _check_fault_states_snapshot(site_identifier: str | None) -> tuple[bool, str]:
    """Compare current /faults/active output to the scheduled fault mode (flatline/bounds/normal)."""
    sched_mod = _get_fault_schedule_module()
    if sched_mod is None:
        return False, "fault_schedule module not available (fake_bacnet_devices missing?)"
    try:
        mode = sched_mod.scheduled_mode()
    except Exception as exc:
        return False, f"Could not determine scheduled fault mode: {exc}"
    expected_flag = _fault_id_for_mode(mode)
    params = {"site_id": site_identifier}
    qs = urlencode({k: v for k, v in params.items() if v not in (None, "")})
    path = "/faults/active"
    if qs:
        path = f"{path}?{qs}"
    ...
    if code != 200 or not isinstance(body, list):
        return False, f"GET /faults/active -> {code}"
    active_faults = {
    ...
    if expected_flag:
        if expected_flag in active_faults:
            return True, f"{expected_flag} active as scheduled ({mode})."
        return False, f"Expected {expected_flag} active during {mode}, active faults={sorted(active_faults)}"
    unexpected = sorted(active_faults & EXPECTED_FAULT_IDS)
    if unexpected:
        return False, f"Scheduled normal window but faults still active: {unexpected}"
    return True, "No scheduled faults active (normal window)."
...
def main() -> int:
    ...
    site_hint = _get_cli_value("--site") or SITE_HINT_ENV or None
    ...
    try:
        ...
        if code != 200:
            ...
            return 1
        print(f"API OK: {api_base}  (interval test)")
        if API_KEY:
        ...
        try:
            site_id, site_name = _resolve_primary_site(site_hint)
        ...
        while True:
            ...
            for interval_min, duration_sec, min_ts, max_ts in selected_phases:
                ...
                count_before, _ = get_download_csv_distinct_timestamps(SITE_QUERY_VALUE, start_d, end_d)
                ...
                count_after, preview = get_download_csv_distinct_timestamps(SITE_QUERY_VALUE, start_d, end_d)
                ...
                if check_faults:
                    ...
                    fault_ok, fault_msg = _check_expected_faults_for_phase(phase_end_utc, SITE_QUERY_VALUE)
                    print(f"  Expected-fault check (API): {'OK' if fault_ok else 'FAIL'} — {fault_msg}")
                    state_ok, state_msg = _check_fault_states_snapshot(SITE_QUERY_VALUE)
                    print(f"  Fault snapshot (active state): {'OK' if state_ok else 'FAIL'} — {state_msg}")
