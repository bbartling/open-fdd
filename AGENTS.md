...
**Offline AHU Typst (single system or building folder):** `open-fdd-anomaly report <folder> --out <dir> --month YYYY-MM` (`--scope single-system|building`). Fixture example `--month 2026-06`. Web OAT is a mapped `web-outside-air-temp` column, `--web-oat weather.csv`, or `--web-oat fetch --lat --lon` (`open_fdd.analytics.open_meteo`). PDF order for facility readers: sensor-check bullets, then a plain anomaly pass/fail list (“looks normal” / “needs a look” / “skipped — not enough fan-on data”), then an executive summary that opens with those two outcomes, then one fan-on week of AHU RCx lines from `charts` / `rcx_plots` / `economizer_temps_overlay`, then `rule_result_chart` only for non-SV rules with confirmed hours, then `economizer_delta_scatter` with **x = OAT − RAT** (`delta_or_f`), **y = MAT − RAT** (`delta_mr_f`), viewport `bottom_left` (both deltas ≤ 0). Do not plot OAT−MAT vs RAT−MAT. Do not put histograms, scoreboards, or anomaly method names in this PDF. This does **not** replace the BUILDING_100 Overview-mirrored lab PDF. Skill: [`openfdd_agent_spec/skills/openfdd-typst-rcx-report/SKILL.md`](openfdd_agent_spec/skills/openfdd-typst-rcx-report/SKILL.md).
