...
**Offline AHU Typst (single system or building folder):** the only shipped PDF command is `open-fdd-anomaly report <folder> --out <dir> --month YYYY-MM --compile` (`--scope single-system|building`). Do not use `build_april_report.py` or any other out-of-tree runner, and do not compile `report.typ` by hand. Default title is **Open-FDD AI Agent Report** (`--title` overrides it). `--location` is an optional site line (example `AHU · ACME Office · Detroit, MI`). The PDF chrome is the Open-FDD blue (`#2563eb` / `#1e3a8a`) and lists month, sample count, sample interval, and span. It works with any AI agent that can read markdown skills and run the PyPI CLI. `openfdd_agent_spec/skills/` is the only source of truth for Open-FDD agent skills. Do not author a parallel skill under `.cursor/skills/` or any other vendor directory. `./scripts/openfdd_install_agent_skills.sh --sync` is the sync into Cursor, Claude, Codex, OpenClaw, Hermes, and Grok. Related skills: typst report, pypi-oracle, rcx-fdd-plot-poll, package-mapping. Fixture example `--month 2026-06`. April device-folder example (do not fetch it in CI): `--month 2026-04 --web-oat ./open_meteo_april.csv --week 2026-04-06` for an 8640-row 5-minute `history_wide.csv` (`2026-04-01` through `2026-04-30`) and a 15-minute Open-Meteo sidecar (`web_oa_t` / `web-outside-air-temp`, reindexed with `align_to_index`). That week is the recommended RCx window through `2026-04-12`. Web OAT otherwise is a mapped `web-outside-air-temp` column, `--web-oat weather.csv`, or `--web-oat fetch --lat --lon` (`open_fdd.analytics.open_meteo`, then `prefer_web_oat`). PDF order for facility readers: a sensor-check paragraph (failed checks only, grouped into sentences), then an anomaly paragraph (“look normal” / “needs a look” / skipped when fan-on data is thin), then an executive summary that opens with those two paragraphs, then one fan-on week of AHU RCx lines from `charts` / `rcx_plots` / `economizer_temps_overlay` / `bas_vs_web_oat_overlay`, then `rule_result_chart` only for non-SV rules with confirmed hours, then `economizer_delta_scatter` with **x = OAT − RAT** (`delta_or_f`), **y = MAT − RAT** (`delta_mr_f`), viewport `bottom_left` (both deltas ≤ 0). Point builder is `build_economizer_delta_points` (`economizer_delta_frame` is the same function). Do not plot OAT−MAT vs RAT−MAT. Do not put histograms, scoreboards, or anomaly method names in this PDF. The template is `open_fdd.reporting.report_template`: `vav_ahu` and `cv_ahu` share the air-side figures (`unitVentilator` / `uv` → `cv_ahu`). Single-zone, chiller, boiler, heat pump, VAV box, fan coil, geothermal field, and data hall are registered stubs. History is mapped roles and is not Railway-only: a local CSV plus column map, a Railway or self-hosted Open-FDD central (agent-supplied reader), or a future vendor API. Agents may add `ai_comments.json`. The PDF has no re-run footer. RCx keeps the economizer rainbow (OAT/RAT/MAT/SAT plus damper percent) and adds fan-on supply-air vs web outdoor air plus a fan-on duct-static box. It does not repeat `ahu_dats`, `ahu_mats`, `ahu_rats`, `ahu_dampers`, `ahu_cooling_valves`, or `fan_speeds`. `--compile` writes `report.pdf` when `typst` is on `PATH`. The economizer scatter caption (`econ_scatter_caption`) sits under the figure: damper color against the outdoor-air fraction lines, damper position is not fresh-air fraction, and mild weather where MAT ≈ RAT ≈ OAT shrinks the deltas. This path does not read `/home/ben/building100_rcx_report` and does **not** replace the BUILDING_100 Overview-mirrored lab PDF. Skill: [`openfdd_agent_spec/skills/openfdd-typst-rcx-report/SKILL.md`](openfdd_agent_spec/skills/openfdd-typst-rcx-report/SKILL.md).
...
**Agent ops (bensbench):** use the **Railway CLI** for backup + hub tip re-pin — [`openfdd_agent_spec/skills/openfdd-railway-cli/SKILL.md`](openfdd_agent_spec/skills/openfdd-railway-cli/SKILL.md) · [`docs/operations/RAILWAY_DEPLOYMENT.md`](docs/operations/RAILWAY_DEPLOYMENT.md). Do **not** confuse Railway CLI / Railway MCP with **`openfdd-mcp`** FDD tools. **FQ OPS PINNED:** **`sha-7ad6479`** / **3.5.43** (Wave U V6 · stress `20260921T234148Z`). Rollback **`sha-1677c33`** / **3.5.37** (prior Wave U FQ) or Soft Tip B **`sha-4a5c11e`** / **3.5.28**.
