...
- Assign FDD rules by setting points[].fdd_rule_ids (array of rule ids from fdd_rules[].id) and/or updating fdd_rules[].bindings.point_ids.
- Export includes points[].fdd_rules_linked [{id,name}] — use those names when explaining assignments; import still uses fdd_rule_ids only.
- fdd_rules[].name matches Rule Lab dropdown labels; source_file links to the deployed .py when present.
