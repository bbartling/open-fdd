...
export function OpenFddClawAdvancedPanel() {
  ...
  const cronValidation: CronValidation = useMemo(() => validateCronExpression(draft.schedule), [draft.schedule]);
  ...
  const cronExamples = useMemo(() => ["@hourly", "0 7 * * *", "0 */6 * * *", "*/15 * * * *", "0 9 * * 1-5"], []);
  ...
  return (
    <div className="card" data-testid="ofdd-claw-advanced-panel">
      ...
      <div className="openclaw-actions">
        ...
        <button type="button" className={`theme-btn ${shell === "posix" ? "active" : ""}`} onClick={() => setShell("posix")}>
          POSIX (bash)
        </button>
        <button
          type="button"
        ...
        <button type="button" className={`theme-btn ${mode === "commands" ? "active" : ""}`} onClick={() => setMode("commands")}>
          Commands
        </button>
        <button type="button" className={`theme-btn ${mode === "api" ? "active" : ""}`} onClick={() => setMode("api")}>
          Create via API
        </button>
      </div>
      <div className="grid-two">
        ...
        <label>
          Cron schedule
          <input value={draft.schedule} onChange={(e) => setDraft((prev) => ({ ...prev, schedule: e.target.value }))} />
        </label>
        <label>
          Timezone
          <input value={draft.tz} onChange={(e) => setDraft((prev) => ({ ...prev, tz: e.target.value }))} />
        </label>
        <label>
          Session type
          <select
            value={draft.session}
            onChange={(e) => setDraft((prev) => ({ ...prev, session: e.target.value as "isolated" | "main" }))}
          >
      ...
      <div className="openclaw-actions">
        ...
        {cronExamples.map((example) => (
          <button key={example} type="button" className="secondary-btn" onClick={() => setDraft((prev) => ({ ...prev, schedule: example }))}>
            {example}
          </button>
        ))}
        <button
      ...
      {mode === "api" ? (
        <div className="stack-page">
          <label>
            Endpoint preset
            <select value={apiEndpointPath} onChange={(e) => setApiEndpointPath(e.target.value)}>
          ...
          <label>
            API endpoint path
            <input value={apiEndpointPath} onChange={(e) => setApiEndpointPath(e.target.value)} placeholder="api/cron/jobs" />
          </label>
          <p className="muted">
            {OPENCLAW_CRON_ENDPOINT_PRESETS.find((p) => p.endpointPath === apiEndpointPath)?.notes ?? "Custom endpoint path."}
          </p>
          <label>
          ...
          <div className="openclaw-actions">
            ...
            <span className="muted">Uses VITE_OPENFDDCLAW_GATEWAY_BASE (or legacy VITE_OPENCLAW_GATEWAY_BASE) + endpoint path.</span>
          </div>
          <label>
          ...
          <div className="openclaw-actions">
            <button type="button" className="secondary-btn" onClick={() => void copyText("api-preview", apiPreview)}>
        ...
        <>
          ...
          <div className="grid-two">
            <label>
              ...
              <div className="openclaw-actions">
                <button type="button" className="secondary-btn" onClick={() => void copyText("cron-clean", buildCronCleanupCommand())}>
            ...
            <label>
              ...
              <div className="openclaw-actions">
                <button type="button" className="secondary-btn" onClick={() => void copyText("skills", buildSkillsRefreshCommands())}>
      ...
      <div className="grid-two">
        <label>
          ...
          <div className="openclaw-actions">
            <button type="button" className="secondary-btn" onClick={() => void copyText("route-preset", SIMPLE_COMPLEX_POLICY_PRESET)}>
        ...
        <label>
          ...
          <div className="openclaw-actions">
            <button type="button" className="secondary-btn" onClick={() => void copyText("security-preset", SECURITY_SAFE_DEFAULTS_PRESET)}>
      ...
      <h3 className="title" style={{ marginTop: "1.5rem" }}>
        Phase 3 memory + multi-site presets
      </h3>
      <div className="grid-two">
        <label>
