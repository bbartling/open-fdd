...
Copy a worked example from ``app.rules.custom_boilerplate`` (shim → PyPI), adapt it,
and append the ``CookbookRule`` to ``CUSTOM_RULES`` below. Ids must start with
``CUSTOM-``.

Leave this list empty to run only the canonical cookbook rules.
...
# from open_fdd.rules.custom_boilerplate import EXAMPLE_SAT_HIGH
