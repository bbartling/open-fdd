...
class FaultCodeTenReport:
    ...
    def summarize_fault_times(self, df: pd.DataFrame, output_col: str = None) -> dict:
        ...
        summary = {
            ...
            'hours_fc10_mode': round((delta * df[output_col]).sum() / pd.Timedelta(hours=1)),
    ...
    def display_report_in_ipython(self, df: pd.DataFrame, output_col: str = "fc10_flag"):
        print("Fault Condition 10: Outdoor air temperature and mix air temperature should be approximately equal in economizer plus mech cooling mode")
