...
class FaultCodeOneReport:
    ...
    def summarize_fault_times(self, df: pd.DataFrame, output_col: str = None) -> dict:
        ...
        summary = {
            ...
            'hours_fc1_mode': round((delta * df[output_col]).sum() / pd.Timedelta(hours=1)),
            'percent_true': round(df[output_col].mean() * 100, 2),
            'percent_false': round((100 - round(df[output_col].mean() * 100, 2)), 2),
            'flag_true_duct_static': round(df[self.duct_static_col].where(df[output_col] == 1).mean(), 2),
            'flag_true_duct_static_spt': round(df[self.duct_static_setpoint_col].where(df[output_col] == 1).mean(), 2),
    ...
    def display_report_in_ipython(self, df: pd.DataFrame, output_col: str = "fc1_flag"):
        ...
        if fc_max_faults_found != 0:
            ...
            print("Duct Static Setpoint Mean When In Fault: ", summary['flag_true_duct_static_spt'])
