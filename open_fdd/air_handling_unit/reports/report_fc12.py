...
class FaultCodeTwelveReport:
    ...
    def summarize_fault_times(self, df: pd.DataFrame, output_col: str = None) -> dict:
        ...
        summary = {
            ...
            'hours_fc12_mode': round((delta * df[output_col]).sum() / pd.Timedelta(hours=1)),
    ...
    def display_report_in_ipython(self, df: pd.DataFrame, output_col: str = "fc12_flag"):
        print("Fault Condition 12: Supply air temperature too high; should be less than mix air temperature in economizer plus mech cooling mode")
