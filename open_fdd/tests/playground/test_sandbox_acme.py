...
def _occupied_cfg(extra: dict | None = None) -> dict:
    base = {
        "tz_offset_hours": 0,
        "occupied_start_hour": 0,
        "occupied_end_hour": 23,
    }
    if extra:
        base.update(extra)
    return base
...
def _synthetic_zone_table(n: int = 70, *, temp: float = 70.0, last_temp: float | None = None) -> pa.Table:
    ...
    if last_temp is not None:
        temps[-1] = last_temp
    stamps = [f"2024-01-01T12:{min(i, 59):02d}:00Z" for i in range(n)]
    return pa.table(
        {
            "timestamp": pa.array(stamps),
...
def test_vav_zone_oob_arrow_flags_high_temp():
    src = (RULES_DIR / "vav_zone_temp_bounds_occupied.py").read_text()
    table = _synthetic_zone_table(5, temp=72.0, last_temp=90.0)
    cfg = _occupied_cfg({"bounds_low": 65, "bounds_high": 80, "value_column": "zone_t"})
    result = run_arrow_rule(src, table, cfg)
    assert not result.errors
    assert result.true_count >= 1


def test_vav_flatline_arrow_imports_cookbook():
    mod = _load_rule_module("vav_zone_temp_flatline_occupied")
    assert hasattr(mod, "apply_faults_arrow")
    table = _synthetic_zone_table()
    cfg = _occupied_cfg({"flatline_tolerance": 0.5, "value_column": "zone_t"})
