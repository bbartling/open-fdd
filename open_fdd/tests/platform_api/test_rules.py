...
def test_rules_sync_definitions():
    ...
    with patch(
        "open_fdd.platform.loop.sync_fault_definitions_from_rules_dir", lambda: None
    ):
...
def test_rules_test_inject_and_delete_when_allowed(tmp_path: Path):
    ...
    tmp_path.mkdir(parents=True, exist_ok=True)
    with (
        patch("open_fdd.platform.api.rules._ALLOW_TEST_RULES", True),
        patch("open_fdd.platform.api.rules._rules_dir_resolved", return_value=tmp_path),
    ):
        r = client.post(
            "/rules/test-inject",
            json={
