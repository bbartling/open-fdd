...
import pandas as pd
from open_fdd.air_handling_unit.faults.helper_utils import process_all_faults
...
df_combined, fault_counts_df = process_all_faults(df, config_dict)


fault_counts_df.to_csv("fault_counts.csv", index=False)
df_combined.to_csv(config_dict["AHU_NAME"] + "_final.csv", index=True)
print("all done")

'''
TEST SAVING REPORT NAMES as
config_dict = {
    # used for report name
    'AHU_NAME': "MZVAV_1"
'''
