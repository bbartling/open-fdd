...
class MechanicalCoolingTracker:
    ...
    def create_plots(self, df: pd.DataFrame):
        ...
        filtered_df['time_delta'] = filtered_df.index.to_series().diff().dt.total_seconds().fillna(0)
        filtered_df = filtered_df[filtered_df['time_delta'] > 0]

        expected_interval = filtered_df['time_delta'].mode()[0]
        filtered_df['time_delta'] = filtered_df['time_delta'].apply(lambda x: expected_interval if x > expected_interval * 2 else x)

        motor_run_time_per_day_seconds = filtered_df.groupby(filtered_df.index.date)['time_delta'].sum()
        motor_run_time_per_day_hours = motor_run_time_per_day_seconds / 3600

        self._display_plot(daily_stats['mean'], 'Daily Avg AHU Duct Static Press', 'Inches WC')
        self._display_plot(daily_stats['std'], 'Daily Std AHU Duct Static Press', 'Inches WC')
        self._display_plot(motor_run_time_per_day_hours, 'AHU Motor Run Time per Day', 'Run Time (Hours)', color='orange')
    ...
    def summarize_fault_times(self, df: pd.DataFrame) -> dict:
        ...
        summary = {
            ...
            'hours_motor_runtime': round(delta.sum() / 3600, 2)  # Convert seconds to hours
        }
    ...
    def _identify_mode(self, row):
        if row[self.oat_col] > 40:
            if row[self.ma_dampers_col] > self.economizer_damper_position and row[self.cw_valve_col] > self.mechanical_valve_position:
                return 'Economizer_plus_Mech'
            elif row[self.ma_dampers_col] > self.economizer_min_oa_pos and row[self.cw_valve_col] < self.mechanical_valve_position:
                return 'Economizer'
            elif row[self.ma_dampers_col] <= self.economizer_min_oa_pos and row[self.cw_valve_col] > self.mechanical_valve_position:
                return 'Mechanical'
    ...
    def _display_mode_plot_with_oat(self, mode_percentages: pd.DataFrame, df: pd.DataFrame):
        daily_avg_oat = df[self.oat_col].resample('D').mean()
        combined_df = mode_percentages.join(daily_avg_oat, rsuffix='_OAT')

        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 12))

        combined_df.iloc[:, :-1].plot(kind='bar', stacked=True, ax=ax1, colormap='viridis')
        ax1.set_title('Mode Percentage')
        ax1.set_ylabel('Mode Percentage (%)')
        ax1.set_xticks([])

        ax2.plot(combined_df.index, combined_df['OaTemp'], color='red', linestyle='-', marker='o')
