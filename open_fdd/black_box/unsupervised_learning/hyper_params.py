...
group_heat_vlv_cntrl = data[['timestamp', 'HW_Valve', 'DischargeTemp', 'Ma_Temp', 'Sa_FanSpeed']]
group_cool_vlv_cntrl = data[['timestamp', 'CW_Valve', 'DischargeTemp', 'Ma_Temp', 'Sa_FanSpeed']]
group_econ_cntrl = data[['timestamp', 'MaDampers', 'RA_Temp', 'Ma_Temp', 'OaTemp', 'Sa_FanSpeed']]
...
def detect_faults(data):
    data['fault_heat_valve'] = ((data['HW_Valve'] > 0) & (data['DischargeTemp'] <= data['Ma_Temp']) & (data['Sa_FanSpeed'] > 0)).astype(int)
    data['fault_cool_valve'] = ((data['CW_Valve'] > 0) & (data['DischargeTemp'] >= data['Ma_Temp']) & (data['Sa_FanSpeed'] > 0)).astype(int)
    data['fault_economizer'] = ((data['MaDampers'] > 0) & (data['RA_Temp'] <= data['Ma_Temp']) & (data['OaTemp'] < data['Ma_Temp']) & (data['Sa_FanSpeed'] > 0)).astype(int)
    return data
...
for group_name, group in groups.items():
    ...
    grid_search = GridSearchCV(estimator=model, param_grid=param_grid, scoring=anomaly_silhouette_score, cv=5, n_jobs=-1, verbose=2)
