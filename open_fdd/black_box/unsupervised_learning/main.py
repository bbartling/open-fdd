...
data["anomaly"] = data["anomaly"].map(
    {1: 0, -1: 1}
)  # Convert -1 (outlier) to 1 for easier interpretation
...
for i, column in enumerate(columns_to_analyze):
    axes[i].boxplot(
        [normal_data[column], anomaly_data[column]], tick_labels=["Normal", "Anomaly"]
    )
...
ax.plot(
    anomaly_data["timestamp"], anomaly_data["chiller_delta_temp"], "ro", markersize=5
)
...
for anomaly_date in anomaly_dates:
    ...
    combined_data = data[
        (data["timestamp"].dt.date >= day_before.date())
        & (data["timestamp"].dt.date <= day_after.date())
    ]
    ...
    for column in group1:
        axes[0].plot(
            combined_data["timestamp"], combined_data[column], label=f"{column}"
        )
        # Highlight anomaly points
        anomaly_points = combined_data[combined_data["anomaly"] == 1]
        if not anomaly_points.empty:
            axes[0].plot(
                anomaly_points["timestamp"], anomaly_points[column], "ro", markersize=5
            )
    ...
    axes[4].plot(
        ...
        combined_data["chiller_delta_temp"],
        label="Chiller Delta Temp",
    )
    if not anomaly_points.empty:
        axes[4].plot(
            anomaly_points["timestamp"],
            anomaly_points["chiller_delta_temp"],
            "ro",
            markersize=5,
        )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, f"all_metrics_around_anomaly_{anomaly_date}.png")
    )
    plt.close()
