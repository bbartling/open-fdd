...
class FaultCodeLowDeltaTReport:
    ...
    def create_plot(self, df: pd.DataFrame, output_col: str = None):
        ...
        ax1.plot(
            df.index, df[self.chw_supply_temp_col], label="CHW Supply Temp", color="b"
        )
        ax1.plot(
            df.index, df[self.chw_return_temp_col], label="CHW Return Temp", color="r"
        )
        ax1.legend(loc="best")
    ...
    def summarize_fault_times(self, df: pd.DataFrame, output_col: str = None) -> dict:
        ...
        summary = {
            ...
            "hours_low_delta_t": round(
                (delta * df[output_col]).sum() / pd.Timedelta(hours=1)
            ),
            "percent_true": round(df[output_col].mean() * 100, 2),
            "percent_false": round((100 - round(df[output_col].mean() * 100, 2)), 2),
            "flag_true_delta_t": round(
                df["delta_t"].where(df[output_col] == 1).mean(), 2
            ),
            "hours_motor_runtime": round(
                (delta * df[self.chw_flow_rate_col].gt(0.01).astype(int)).sum()
    ...
    def display_report_in_ipython(
        self, df: pd.DataFrame, output_col: str = "low_delta_t_flag"
    ):
