...
'''
Lagged Features: Created lagged versions of each column to 
incorporate temporal dependencies.

Seasonality Features: Added hour, day of the week, and month 
as features to capture seasonal effects.

Time-Based Cross-Validation: Use a manual train-test split while 
ensuring that the test set follows the train set chronologically.
...
df = create_lagged_features(df, lag=3)
