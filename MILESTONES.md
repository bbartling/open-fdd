...
| --- | --- | --- | --- |
| **U-A — Evaluated qualification** | Security/test maintainer | IN PROGRESS (3.5.36 tip) | Permanent negatives for 14 audit false-passes; both gate 36 required; railway_field candidate/artifact validation; tenant_ids membership; MQTT require-live; ZAP fail-closed; Nessus per-host completeness. CI + MEGA still required for VERIFIED. |
| **U-B — Standalone OT readiness** | Deployment/security maintainer | REOPENED | Whole app works on one supported Linux host without cloud integration; verified HTTPS and peer exposure; no management bypass; host/image/runtime controls accepted; install/restart/restore documented and tested. |
| **U-C — Field gateway readiness** | Field/MQTT maintainer | REOPENED | Field-only recipe and exposure profile; real generated identity/ACL/key lifecycle; measured tenant/command denial and allowed delivery; host hardening; reconnect/spool tests with simulated OT. |
...
| **U-E — Image and host acceptance** | Release/deployment maintainer | REOPENED | Digest/SBOM/DB-version evidence for all deployed components, including TLS front end; required findings remediated; per-finding Medium dispositions; host/runtime checks for both OT profiles. Scan acquisition alone does not close this milestone. |
