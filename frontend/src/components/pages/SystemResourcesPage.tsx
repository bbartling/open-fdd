...
export function SystemResourcesPage() {
  ...
  return (
    <div>
      ...
      {host && (
        <div className="mb-8">
          <h2 className="mb-3 text-sm font-medium text-muted-foreground">Host ({host.hostname})</h2>
      ...
      {disks.length > 0 && (
        <div>
          <h2 className="mb-3 text-sm font-medium text-muted-foreground">Disk usage</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {disks.map((d) => (
              <Card key={`${d.hostname}-${d.mount_path}`}>
                <CardContent className="pt-6">
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    <HardDrive className="h-4 w-4" />
                    {d.hostname} — {d.mount_path}
                  </div>
                  <p className="mt-1 text-2xl font-semibold tabular-nums">
                    {d.used_gb} / {d.total_gb} GB
                  </p>
                  <p className="mt-0.5 text-sm text-muted-foreground">
                    {d.used_pct}% used · {d.free_gb} GB free
                  </p>
                  <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-muted">
                    <div
                      className={`h-full rounded-full ${
                        d.used_pct >= 90 ? "bg-destructive" : d.used_pct >= 80 ? "bg-yellow-500" : "bg-primary"
                      }`}
