import pandas as pd
import pytest
from air_handling_unit.faults.fault_condition_twelve import FaultConditionTwelve
...
class TestFaultConditionTwelve:
    ...
    def test_fault_in_mech_clg(self):
        ...
        actual = results['fc12_flag'].sum()
        expected = 2
        message = f"FC12 fault_df_in_mech_clg actual is {actual} and expected is {expected}"
        assert actual == expected, message
