...
| PERF-1 | §6 | Baseline→candidate measurements | S5 | bench script | reports table | | | Soft-OPEN |
| EQ-VOCAB | §7.1 | Engineering quantity vocabulary + shapes | S5 | `docs/modeling/` + `.ttl` | Pages build | | | Soft-OPEN |
| EQ-PERSIST | §7.2 | Import→export retain capacities | S5 | package/importer | save/reload fixtures | | | Soft-OPEN |
| ECM-ADAPT | §7.3 | Model→`open_fdd.ecm_engineering` adapter | S3/S5 | PyPI tools | fan/schedule/kw_ton cases | | | Soft-OPEN |
| DOCS-PAGES | §8 | GitHub Pages vocab/onboarding/agent | S5 | `docs/` + workflow | jekyll + public URL | | | Soft-OPEN |
