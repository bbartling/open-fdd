...
| DM-09 | §2/§6 | Bound rebuild/query CPU/RAM; measure first | S5 | oxigraph path | bench table § PERF-1 | | | **Soft-OPEN** (W4: no product Rust PERF tip; measure-first residual) |
| DM-10 | §2 | Versioned projection; no false markers / lossless claims | S5 | EQ-VOCAB `ofdd_engineering_quantities_v1` + Pages honesty docs | `pytest tests/ecm_engineering/test_eq_vocab_adapter.py` | tip W4 / 4.4.7 | vocab+adapter tests | **PARTIAL** (EQ projection CLOSED; edge Haystack `projectionVersion` still Soft-OPEN) |
...
| PERF-1 | §6 | Baseline→candidate measurements | S5 | bench script | reports table | | | **Soft-OPEN** (DM-09 dependency) |
