#!/usr/bin/env bash
#
# Open-F-DD — bootstrap: VOLTTRON via official Docker (volttron-docker) + optional local Timescale for SQL schema.
#
...
usage() {
  cat <<'EOF'
Open-F-DD bootstrap (VOLTTRON in Docker via volttron-docker)
...
Options:
  ...
  --write-env-defaults  Write $VOLTTRON_HOME/openfdd-defaults.env (TZ, OFDD_*, DB DSN, PYTHONPATH for Open-F-DD on host)
...
run_doctor() {
  echo "=== Open-F-DD bootstrap doctor (read-only) ==="
  echo ""
  local fail=0
  ...
  if have_cmd python3; then
    ...
    echo "[WARN] python3 not found (install for local Open-F-DD tooling)"
  fi
...
run_write_env_defaults() {
  ...
  cat >"$target" <<EOF
# Open-F-DD defaults for VOLTTRON Central + SQL FDD (generated by bootstrap.sh).
...
# Host-side Open-F-DD imports (agents / scripts); VOLTTRON itself runs inside the container image.
...
run_verify_fdd_schema() {
  echo "=== Verifying Open-F-DD FDD / CRUD tables (fault rules + results) ==="
  local q
  q="
DO \$\$
DECLARE
...
Host env (optional): $vh/openfdd-defaults.env — TZ, OFDD_*, PYTHONPATH for Open-F-DD (see --write-env-defaults).
...
Open-F-DD UI (host build, then install agent in container):
  ./afdd_stack/scripts/bootstrap.sh --build-openfdd-ui
  ./afdd_stack/scripts/bootstrap.sh --write-openfdd-ui-agent-config
...
run_central_lab() {
  echo "=== Open-F-DD central-lab bundle ==="
  run_compose_db
  if have_cmd docker && docker ps --format '{{.Names}}' 2>/dev/null | grep -qx 'openfdd_timescale'; then
