...
function FaultsTable({
  ...
  function deviceLabel(fault: FaultState): string {
    ...
    if (fault.bacnet_device_id != null && fault.bacnet_device_id !== "") {
      return `${base} (instance ${fault.bacnet_device_id})`;
    }
    return base;
  }
  ...
  if (devices.length === 0) {
    return (
      <div className="py-12 text-center text-sm text-muted-foreground" data-testid="fault-matrix-empty">
        No devices in the fault matrix yet. Add points with equipment and a stable device instance id (when present) to see the matrix.
      </div>
    );
  }
  ...
  return (
    <div className="overflow-x-auto">
      <Table data-testid="fault-matrix-table">
        <TableHeader>
          <TableRow>
            <TableHead className="font-mono">Device instance</TableHead>
