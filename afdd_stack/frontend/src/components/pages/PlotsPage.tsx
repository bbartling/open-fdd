...
export function PlotsPage() {
  ...
  /** All equipment IDs tied to the selected device (one device can span multiple equipment records). */
  ...
  /** Equipment rows tied to the selected device — backend must filter fault_results, not only site + fault_id. */
      ...
      <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Plot device trends with fault overlays.
          </p>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-border/60 bg-card p-4">
            ...
            <select
              ...
              title={
                faultIdsForDevice.length === 0
                  ? "No fault state rows for this device yet. Run FDD or pick another device."
                  : undefined
              }
            >
              {faultIdsForDevice.length === 0 ? (
        ...
        {selectedDeviceId && faultIdsForDevice.length === 0 && (
          <p className="mt-2 text-xs text-muted-foreground">
            Faults listed here come from fault state for this device (equipment link or matching device id on points). If the list is empty, run an FDD job or confirm
        ...
        {traces.length > 0 ? (
          ...
          <div className="flex h-[50vh] min-h-[360px] items-center justify-center rounded-lg border border-dashed border-border bg-muted/20 text-sm text-muted-foreground">
            <span className="inline-flex items-center gap-2">
              ...
              Select a device, choose points, then load data to plot.
            </span>
          </div>
        )}
      </div>
