#!/usr/bin/env bash
# Authenticated AI path: login → import package → create job → WattLab dump → download.
# Usage:
#   ./scripts/agent_wattlab_dump.sh /home/ben/raw_BUILDING_100_openfdd.zip [out_dir]
set -euo pipefail

ZIP="${1:?package zip path required}"
OUT="${2:-/tmp/openfdd_wattlab_dump}"
