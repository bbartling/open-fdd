#!/usr/bin/env python3
"""Compare OpenFDD engineering dump bundles (FDD + analytics tables).

Legacy name: ``wattlab_parity_diff.py`` (vibe19 dual-parity retired).

Tolerances:
  counts exact; medians abs<=0.05 or rel<=0.1%; fault hours abs<=0.05h or <=0.1pp;
  schedule calendar fields exact.

Emits ``reports/eplus-dump/artifacts/diff_summary.json`` (+ markdown rows).
"""
...
try:
    ...
    _ART = parity_root() / "artifacts"
except ImportError:
    _ART = ROOT / "reports/wattlab-parity/artifacts"

DEFAULT_ORACLE = _ART / "vibe19_oracle"
DEFAULT_OFDD = _ART / "ofdd_rust_bundle"
DEFAULT_CAPTURE = _ART / "ofdd_rust"
DEFAULT_OUT = _ART / "diff_summary.json"
