...
52. **SPA copy discipline:** Product UI stays quiet. Agent rationale belongs in `openfdd_agent_spec` / PR bodies — never goofy in-app tip strips. Skill: [`openfdd-react-spa`](skills/openfdd-react-spa/SKILL.md). Wave O7 plan: `.cursor/plans/wave_o7_overview_site_cache_ce235993.plan.md`.
...
55. **Keep this spec current:** any PR that changes demo freshness, site catalog scoping, Inspect/MQTT plots, Overview tables, Data Model export, Lakeside/FDD planning, plot date-span defaults, Railway closeout workflow, **Wave L tenant/Parquet isolation**, **Kali O2c/P2c pre-auth / MT ACL / CSP/`security.txt`**, or agent tooling (Railway CLI / GHCR / MCP) updates this file + [`SESSION_LOG.md`](SESSION_LOG.md) + [`skills/openfdd-mt-security`](skills/openfdd-mt-security/SKILL.md) in the same train.
...
**Current ops pin (2026-09-14):** VERSION **3.5.10** · tip **`sha-9072e0b`** · health **`3.5.10+9072e0b9fcf6`** · `multi_tenant=true` (authorized Stage C early waiver) · Wave N **OPS PINNED** (data-path `allow_building` + ACME MQTTS). Rollback: **`sha-1f94cdf`** / **3.5.9** or Wave M **`sha-0bfcd81`** / **3.5.7**. Low-RAM always (rule 0). **Note:** #934 O12b merged 2026-09-15 — GHCR/Railway re-pin **deferred** while Kali works hub.
