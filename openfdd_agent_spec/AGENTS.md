...
51. **Demo freshness (buyer UX — Wave H):** Dialing into Overview / FDD / RCx / Inspect must not require mashing **Run all rules** or **Update analytics** on every visit to see data that already exists. Serve **last-good** results with an as-of / stale badge; soft-refresh or scheduled warm for demo sites (`bldg2`, `LAKESIDE_ES`, `BUILDING_100`, `BUILDING_50`). Manual Run remains for deliberate recompute. AFDD flood not-yet-run is OK (honest empty + CTA). Empty matrices/`—` while `ingest_ok` climbs or historian has points is a **P0 bug**. Spec + plan: Wave H `post_waveg_openfdd_residual_wave_h`.
52. **Multi-site chart bar:** Lakeside (Creekside → `LAKESIDE_ES`), `BUILDING_100`, `BUILDING_50` (when package present), and MQTT `bldg2` must show **visible charts/data** after Wave H — Inspect + ≥1 FDD series + ≥1 RCx preset where applicable. B50 missing package → honest DEFERRED in BUG_REPORT with obtain path, not silent skip. Railway hub: **0 failed/crashed** central/mqtt/web; fieldbus up; MQTT ingest climbing.
...
54. **Keep this spec current:** any PR that changes demo freshness, site catalog scoping, Inspect/MQTT plots, Overview tables, Data Model export, Lakeside/FDD planning, plot date-span defaults, Railway closeout workflow, **Wave L tenant/Parquet isolation**, or agent tooling (Railway CLI / GHCR / MCP) updates this file + [`SESSION_LOG.md`](SESSION_LOG.md) in the same train.
55. **Basic app functionality (Wave I — non-negotiable):** Selecting a packaged or MQTT site must not explode FDD (`read_csv` / planning errors). Overview main-tab tables (especially bottom devices table) must **never** be filtered differently for MQTT vs CSV — same chrome; empty = honest zero/shell. Data Model must offer **Export data model JSON** that works for MQTT and CSV. Do not close Wave I on stress green if these SPA/API checks fail.
56. **Default plot span:** Overview / Inspect / RCx defaults must cover the historian’s available `first_timestamp`→`last_timestamp` (even downsample to `max_points`). Newest-only `LIMIT` that hides earlier months (e.g. B100 July-only) is a **P0 MEGA**.
...
59. **Wave L multi-tenant (mode OFF until Tier-2):** `OPENFDD_MULTI_TENANT` defaults OFF. L1: `TenantContext` + `/api/tenants` + gate 11. L2: tenant Parquet roots via `fdd_store::tenant_storage_root` / `TenantContext::historian_root` (hub root when OFF; `{base}/tenants/{tid}` when ON) + gate 12. L3: MQTTS `TopicBuilder` / `parse_topic` — legacy `openfdd/v1/sites/{site}/edges/{edge}/…` when OFF; `openfdd/v1/tenants/{tid}/buildings/{bid}/edges/{eid}/…` when ON; ingest rejects envelope≠topic and rejects legacy topics when mode ON; gate 13. L4: single-domain tenant session — `/api/auth/me` + login echo `active_tenant_id`/`multi_tenant`; `POST /api/tenants/select` (OFF = legacy no-op); SPA sidebar `app-tenant` + Auth session Tenant row; gate 14. L5: per-tenant budgets (`OPENFDD_TENANT_BUDGETS`) disabled unless mode ON; `/api/tenants/budgets` + FDD/jobs enforcement; lab notes [`WAVE_L_TENANT_BUDGETS_LAB.md`](../docs/operations/WAVE_L_TENANT_BUDGETS_LAB.md); gate 15. L6: Tier-2 A/B isolation harness + tip digest scan + disposable ZAP AF keeps mode OFF (`scripts/qualification/wave_l_ab_isolation_harness.sh`, gate 16). L7: legacy-tenant migrate dry-run + checklist (`scripts/ops/wave_l_legacy_migrate_dry_run.sh`, [`WAVE_L_LEGACY_MIGRATE_CHECKLIST.md`](../docs/operations/WAVE_L_LEGACY_MIGRATE_CHECKLIST.md), gate 17) — no silent APPLY on Railway. Isolation = **path-scoped DF providers**, never shared mega-table + `WHERE tenant_id`. ADR: [`docs/architecture/ADR_multi_client_shared_hosting.md`](../docs/architecture/ADR_multi_client_shared_hosting.md).
...
**Current ops pin (2026-09-12):** VERSION **3.5.7** · tip **`sha-0bfcd81`** · health **`3.5.7+0bfcd81b949f`** · `multi_tenant=false` · `active_tenant_id=legacy` · `tenant_budgets=false` · Wave M **OPS PINNED** (M5 Mint stress partial — see BUG_REPORT Soft-OPEN). Rollback: Wave L **`sha-e80237c`** / **3.5.6** or Wave K **`sha-9c3e8b1`** / **3.4.0**. Low-RAM always (rule 0).
