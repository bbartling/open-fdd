...
**Skills hard rule:** `openfdd_agent_spec/skills/` is the only authoring tree for Open-FDD AI agent skills. Grok Bot, Hermes, OpenClaw, Cursor, Claude, and Codex build from that tree. Do not invent a second copy under `.cursor/skills/`, `.claude/skills/`, `.agents/skills/`, or a home directory. Those paths are sync targets of [`scripts/openfdd_install_agent_skills.sh`](../scripts/openfdd_install_agent_skills.sh) (`--sync`, optional `--user`). Edit the file in `skills/`, then sync.
...
61. **Agent tooling map (use the right tool):** GHCR Publish / tip gate = GitHub Actions + `scripts/check_ghcr_tip_stack.sh`. Hub re-pin / backup / ssh health = **Railway CLI** (rule 39). Local lab stack = `openfdd_stack_*.sh` (HTTP firewall only). FDD/analytics for AI hosts = **`openfdd-mcp`** + agent JWT — not Railway MCP. Stress = `scripts/nightly-ot-bench/`. Typst RCx PDFs = [`skills/openfdd-typst-rcx-report`](skills/openfdd-typst-rcx-report/SKILL.md). BUILDING_100 Overview mirror stays the legacy kit. Offline single-system or building-folder AHU packs use only `open-fdd-anomaly report ... --compile` (`open_fdd.reporting.single_system_typst`) and must not overwrite that kit. Do not use `build_april_report.py` or any other out-of-tree runner. Default PDF title is **Open-FDD AI Agent Report** (`--title`, `--location`). These skills work with any AI agent that can read markdown skills and run the PyPI CLI. `openfdd_agent_spec/skills/` is the only authoring tree. `./scripts/openfdd_install_agent_skills.sh --sync` fills vendor directories. Do not author under `.cursor/`. That pack's PDF is month-filtered (`--month YYYY-MM`) and reads a sensor-check paragraph, then an anomaly paragraph, then RCx/FDD plots. No histograms, scoreboards, or anomaly method names. Economizer scatter is `economizer_delta_scatter` with viewport `bottom_left`: **x = OAT − RAT** (`delta_or_f`), **y = MAT − RAT** (`delta_mr_f`), via `build_economizer_delta_points` (`economizer_delta_frame` is that same function). Do not plot OAT−MAT vs RAT−MAT. The note under that figure is `econ_scatter_caption`: damper color vs the OA-fraction lines, damper position is not fresh-air fraction, and mild weather (MAT ≈ RAT ≈ OAT) shrinks the deltas. Web OAT comes from a mapped column, `--web-oat` CSV (`web_oa_t` reindexed onto the BAS clock, then `prefer_web_oat`), or `--web-oat fetch`. `--week YYYY-MM-DD` pins the RCx window (April example `2026-04-06` through `2026-04-12`). `--compile` writes `report.pdf` when `typst` is on `PATH`. The single-AHU path does not read `/home/ben/building100_rcx_report`. The template is `open_fdd.reporting.report_template` (`vav_ahu` and `cv_ahu` share the air-side figures; `unitVentilator` / `uv` → `cv_ahu`; single-zone, chiller, boiler, heat pump, VAV box, fan coil, geothermal field, and data hall stay registered stubs). History is mapped roles and is not Railway-only: a local CSV plus column map, a Railway or self-hosted Open-FDD central (agent-supplied reader), or a future vendor API. The PDF omits a re-run footer and the duplicate AHU timeseries presets. Hygiene every merge: **0 open PRs**, remote **only `master`**, tip Actions green (ignore cancelled docs-tip Publish noise).
...
Author only the files in this table. Sibling links inside each skill point at the same tree. `./scripts/openfdd_install_agent_skills.sh --sync` may symlink them into `.cursor/skills/`, `.agents/skills/`, and `.claude/skills/`. A same-named real directory in those folders is a second copy; delete it and sync again.
