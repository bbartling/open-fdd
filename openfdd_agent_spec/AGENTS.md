...
54. **Keep this spec current:** any PR that changes demo freshness, site catalog scoping, Inspect/MQTT plots, Overview tables, Data Model export, Lakeside/FDD planning, plot date-span defaults, or Railway closeout workflow updates this file + [`SESSION_LOG.md`](SESSION_LOG.md) in the same train.
...
**Current ops pin (2026-09-09):** health **`3.3.41+cee2f4ec6c65`** · tip **`sha-cee2f4e`** (K1b) until K5 tip re-pin. Workspace **VERSION 3.4.0** (K5). Wave J **CLOSED**. **Wave K OPEN** — tip Publish → re-pin → K6 stress → PINNED. Then Wave L 3.5 multi-client shared hosting. #782 optional K4. Low-RAM always (rule 0).
