...
fn login(stream: &mut TcpStream, body: &str) -> std::io::Result<()> {
    ...
    if !["operator", "integrator", "agent"].contains(&role) {
        return status_json(
            stream,
            "400 Bad Request",
            json!({"ok": false, "error": "role must be operator, integrator, or agent"}),
        );
    }
    let sub = value
        .get("sub")
        .and_then(|v| v.as_str())
        .unwrap_or("edge-agent");
    let token = create_jwt(sub, role);
    json_response(
        stream,
        json!({"ok": true, "token_type": "Bearer", "access_token": token, "expires_in": 8 * 3600, "role": role}),
    )
}
