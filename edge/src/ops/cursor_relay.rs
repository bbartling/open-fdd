//! HTTP bridge to local Cursor SDK relay (`tools/cursor-chat-relay`).
...
fn env_or_file(key: &str) -> Option<String> {
    ...
    let file = workspace_dir().join("cursor.env.local");
    read_env_file(&file)
        .get(key)
        .cloned()
...
pub fn base_url() -> String {
    env_or_file("OFDD_CURSOR_CHAT_URL")
...
/// Fast health check (~1s) — relay process up and configured.
pub fn probe_quick() -> Option<String> {
    ...
    if body.get("has_api_key").and_then(|v| v.as_bool()) == Some(false) {
        return None;
    }
    Some(url)
}
...
pub fn status_json() -> Value {
    ...
    match client().get(format!("{url}/health")).send() {
        ...
        Err(err) => json!({
            ...
            "base_url": url,
            "error": err.to_string(),
            "hint": "Run ./scripts/openfdd_cursor_chat_relay.sh"
        }),
    }
}
...
pub fn chat(body: &Value) -> Result<Value, String> {
    let base = probe_quick().ok_or_else(|| {
        format!(
            "Cursor relay unreachable at {} — run ./scripts/openfdd_cursor_chat_relay.sh",
            base_url()
        )
    })?;
    let resp = client()
